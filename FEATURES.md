...
| **컬러 선택** | 드랍다운 방식: 블랙/차콜/그레이/베이지/화이트 + 직접입력 (한글 라벨) |
| **직접입력** | 드랍다운에서 "직접입력..." 선택 시 텍스트 입력창 표시 (대문자 자동 변환) |
| **새 컬러 그룹** | 드랍다운으로 추가 (프리셋 선택 또는 직접입력) |
...
| `ProductForm` | 판매자 상품 등록 공통 폼 (신규/복제 등록 공용), 드랍다운 방식 컬러 선택 |
...
### 컬러 선택 개선
- **한글 라벨**: 프리셋 컬러 한글 표시 (블랙, 차콜, 그레이, 베이지, 화이트)
- **드랍다운 방식**: 버튼 나열 → 드랍다운 선택으로 변경
- **직접입력 UI**: 드랍다운에서 "직접입력..." 선택 시 텍스트 입력창 표시
- **자동 대문자 변환**: 입력값 자동 대문자 변환 (NAVY, BROWN 등)
- **일관된 디자인**: 사이즈 입력창과 동일한 높이(h-10) 및 스타일
